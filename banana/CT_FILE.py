...
"""
=============================================================
CT_FILE.py - 单张图片处理工具（完全独立版本）
=============================================================

功能概述:
--------
处理单张图片文件，生成配置并调用自动化任务。
...
def load_previous_config():
    ...
    if not os.path.exists(config_file_path):
        ...
        return last_folder, last_prompt, last_img1, last_img2
        ...
        try:
            with open(config_file_path, 'r', encoding='utf-8') as f:
                ...
                if isinstance(data, dict):
                    ...
                    if configs:
                        last_config = configs[0] if isinstance(configs, list) and len(configs) > 0 else configs
                        if isinstance(last_config, dict):
                            last_img1 = last_config.get('banana_ref_img1', '')
                            last_img2 = last_config.get('banana_ref_img2', '')
        except Exception as e:
...
def get_clipboard_image():
    ...
    try:
        ...
        if clipboard_content is not None:
            if hasattr(clipboard_content, 'mode') and hasattr(clipboard_content, 'size'):
                ...
                try:
                    ...
                    try:
                        ...
                        pass
                    log_message(f"保存剪贴板图片失败: {e}")
                    return None
            elif isinstance(clipboard_content, list):
                for path in clipboard_content:
                    if any(path.lower().endswith(ext.replace('*','')) for ext in SUPPORTED_EXTENSIONS):
                        return path
    except ImportError:
        log_message("PIL库未安装，无法从剪贴板获取图片")
    except Exception as e:
        log_message(f"从剪贴板获取图片失败: {e}")
    return None


def scan_and_generate_config(img1_path, img2_path, headless, model, wait_time, output_folder, prompt):
    """生成配置数据"""
    if not os.path.exists(output_folder):
        os.makedirs(output_folder, exist_ok=True)
    image_filename = os.path.basename(img1_path)
    save_name = os.path.splitext(image_filename)[0]
    config_item = {
        "banana_prompt": prompt,
        "banana_ref_img1": img1_path,
        "banana_ref_img2": img2_path,
        "banana_model": model,
    ...
    config_data = {
        "headless": headless,
        "last_input_folder": os.path.dirname(img1_path),
        "banana_prompt": prompt,
        "configs": [config_item]
    }
    return config_data
...
def get_user_parameters(last_folder, last_prompt, last_img1, last_img2):
    """交互式获取用户参数"""
    print("\n操作选项:")
    print(f"- 复制第一张图片到剪贴板后，输入 P 开始处理")
    print(f"- 或者输入 Q 退出程序")
    choice = input("请输入选择: ").strip().upper()

    if choice == "Q":
        print("程序退出")
        exit(0)
    elif choice != "P":
        print("无效输入，请复制第一张图片到剪贴板后输入 P")
        return get_user_parameters(last_folder, last_prompt, last_img1, last_img2)

    img1_path = get_clipboard_image()
    if not img1_path:
        print("未能从剪贴板获取到第一张图片，请确保已经复制了图片数据")
        return get_user_parameters(last_folder, last_prompt, last_img1, last_img2)

    print(f"检测到第一张图片: {os.path.basename(img1_path) if img1_path else 'N/A'}")
    print("\n现在请复制第二张图片到剪贴板，然后按回车键继续...")
    input("按回车键继续...")

    img2_path = get_clipboard_image()
    if not img2_path:
        print("未能从剪贴板获取到第二张图片，将使用上次的路径或为空")
        img2_path = last_img2

    print(f"检测到第二张图片: {os.path.basename(img2_path) if img2_path else 'N/A'}")

    prompt_input = input(f"请输入提示词 (回车使用上次的提示词: {last_prompt}): ").strip()
    prompt = prompt_input if prompt_input else last_prompt

    print("\n默认模式参数:")
    print("- 浏览器模式: 无头模式 (headless)")
    print(f"- 模型选择: {DEFAULT_MODEL}")
    print(f"- 等待时间: {DEFAULT_WAIT_TIME}秒")
    print(f"- 输出文件夹: {last_folder}_output (来自上次配置)")
    print(f"- 提示词: {prompt}")
    print()
    ...
    output_folder = get_output_folder_path(last_folder) if last_folder else get_output_folder_path(os.path.dirname(img1_path) if img1_path else os.getcwd())
    ...
    return headless, model, wait_time, output_folder, prompt, img1_path, img2_path
...
def main():
    """主程序入口"""
    log_message("程序启动...")
    last_folder, last_prompt, last_img1, last_img2 = load_previous_config()
    headless, model, wait_time, output_folder, prompt, img1_path, img2_path = get_user_parameters(last_folder, last_prompt, last_img1, last_img2)

    if not img1_path or not os.path.isfile(img1_path):
        log_message(f"错误: 第一张图片路径 '{img1_path}' 无效或不存在")
        return

    if img2_path and img2_path != last_img2 and not os.path.isfile(img2_path):
        log_message(f"警告: 第二张图片路径 '{img2_path}' 无效或不存在，将忽略该图片")

    log_message(f"当前处理图片1: {os.path.basename(img1_path) if img1_path else 'N/A'}")
    if img2_path and img2_path != last_img2:
        log_message(f"当前处理图片2: {os.path.basename(img2_path) if img2_path else 'N/A'}")
